...
class MemTracker(object):
    ...
    def __init__(self, frame, detail=True, path='', verbose=False, device=0):
        ...
        self.gpu_profile_fn = path + datetime.datetime.now().strftime('%d-%b-%y-%H:%M:%S') + '-gpu_mem_track.txt'
    ...
    def track(self):
        ...
        with open(self.gpu_profile_fn, 'a+') as f:
            ...
            if self.begin:
                f.write("GPU Memory Track |" +  datetime.datetime.now().strftime('%d-%b-%y-%H:%M:%S') + "|" +
                        " Total Used Memory:{}\n\n".format(meminfo.used/1024**2))
                self.begin = False
            ...
            if self.print_detail is True:
                ...
                for t, s, n, m in new_tensor_sizes - self.last_tensor_sizes:
                    f.write('+ |' + '{:30}'.format(str(n) + ' * ' + 'Size:' + str(s)) + '|' + ' Memory: ' +
                            '{:7.6f}'.format(m*n) + ' | ' + str(t) + '\n')
                for t, s, n, m in self.last_tensor_sizes - new_tensor_sizes:
                    f.write('- |' +  '{:30}'.format(str(n) + ' * Size:' + str(s)) + '| Memory: ' + '{:7.6f}'.format(m*n) + ' | ' +  str(t) + '\n')
                self.last_tensor_sizes = new_tensor_sizes

            f.write('\n' + 'At ' + where_str + '     Total Used Memory:' + '{:20}'.format(meminfo.used/1024**2) + 'Mb' + '\n\n')
